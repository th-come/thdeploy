...
export class Deploy {
	...
	constructor(config: DeployConfigItem, workspaceRoot: string, callback: (success: boolean) => void) {
		...
		this.taskList = [
			...
			{ task: this.connectSSH, tip: "连接服务器", increment: 40 },
			{ task: this.removeRemoteFile, tip: "删除服务器文件", increment: 50, async: false },
			{ task: this.uploadLocalFile, tip: "上传文件至服务器", increment: 60 },
			{ task: this.unzipRemoteFile, tip: "解压服务器文件", increment: 70 },
			{ task: this.delZipFile, tip: "删除远程压缩包", increment: 80 },
			{ task: this.disconnectSSH, tip: "断开服务器", increment: 90, async: false },
			{ task: this.removeLocalZip, tip: "删除本地压缩文件", increment: 100, async: false },
		];
		this.start();
	}
